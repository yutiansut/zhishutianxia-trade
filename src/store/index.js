...
//						}else if(parameters.Status == 2 || parameters.Status == 3 || parameters.Status == 4 || parameters.Status == 5){
			...
			//退出交易账号清除websocket
			if(context.state.account.loginStatus == false && context.state.tradeSocket){
				context.state.tradeSocket.close();
				context.state.tradeSocket = null;
			}
		},
...
//			console.log('根据订阅行情初始化持仓盈亏');
			if(context.state.market.orderTemplist[parameters.CommodityNo].LastQuotation == undefined) return;
